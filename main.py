...
class Poker(object):
    ...
    def isRoyal(self,
                hand):  # returns the total_point and prints out 'Royal Flush' if true, if false, pass down to
        ...
        Currank = 14
        total_point = h * 13 ** 5 + point(sortedHand)
        for card in sortedHand:
    ...
    def isStraightFlush(self,
                        hand):  # returns the total_point and prints out 'Straight Flush' if true, if false,
        # pass down to isFour(hand)
        sortedHand = sorted(hand, reverse=True)
        flag = True
        h = 9
        Cursuit = sortedHand[0].suit
